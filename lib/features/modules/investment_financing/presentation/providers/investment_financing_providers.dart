...
class InvestmentFinancingState {
  ...
  double get totalInvestment =>
      investmentItems.fold(0, (sum, item) => sum + item.amount);

  double get selectedFinancingTotal => financingOptions
      .where((option) => option.isSelected)
      .fold(0, (sum, option) => sum + option.amount);

  bool get hasFinancialGap =>
      totalInvestment > 0 && selectedFinancingTotal < totalInvestment;
...
class InvestmentFinancingNotifier
  ...
  Future<void> completeModule() async {
    ...
    if (state.hasFinancialGap) {
      state = state.copyWith(
        errorMessage:
            'La financiaciÃ³n seleccionada no cubre la inversiÃ³n total. Ajusta la estructura de capital antes de completar.',
        clearSuccessMessage: true,
      );
      return;
    }
