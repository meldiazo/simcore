...
class MarketNotifier extends StateNotifier<AsyncValue<void>> {
  ...
  MarketNotifier(this._repository, this._companyId, this._ref) : super(const AsyncValue.data(null));
  ...
  Future<bool> generateProjection() async {
    ...
    try {
      ...
      state = const AsyncValue.data(null);
      _ref.invalidate(salesProjectionProvider); // Refresca la proyección en la UI
      return true;
    } catch (e, stack) {
      state = AsyncValue.error(e, stack);
      return false;
